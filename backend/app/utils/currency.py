...
CURRENCY_SYMBOLS = {
    '$': 'USD',           # US Dollar (also used in others)
    '€': 'EUR',          # Euro
    '£': 'GBP',          # British Pound
    '¥': 'JPY',          # Japanese Yen
    '₹': 'INR',          # Indian Rupee
...
# Major currencies with conversion rates to USD (base currency)
# In production, these should be fetched from an API like Open Exchange Rates
...
# Common currency patterns in invoice text
CURRENCY_PATTERNS = [
    (r'USD|\$(?!\s)|US\$', 'USD'),
    (r'EUR|€|Euro', 'EUR'),
...
def detect_currency(text: str) -> Tuple[str, float]:
    ...
    """
    if not text:
        return 'USD', 0.5  # Default fallback
...
def format_currency(amount: float, currency_code: str, include_symbol: bool = True) -> str:
    ...
    if include_symbol:
        ...
        # Place symbol before or after depending on convention
        if currency_code in ['EUR', 'GBP', 'JPY', 'CNY']:
